...
import * as UserDTO from "@my_types/user";
import * as AuthDTO from "@my_types/auth";
import ms from "ms";
...
export const register = async (
	...
	const expiresAt = add(new Date(), {
		seconds: COMPUTED.REFRESH_TOKEN_EXPIRY_SECONDS,
	});
 ...
 */
export const login = async (dto: AuthDTO.LoginDTO): Promise<LoginResponse> => {
	const user = await db.User.findOne({
		where: {
			[Op.or]: [{ email: dto.login }, { userName: dto.login }],
		},
	});
	if (!user)
		throw {
	...
	logger.debug(COMPUTED.REFRESH_TOKEN_REMEMBER_EXPIRY_SECONDS);
	logger.debug(COMPUTED.REFRESH_TOKEN_EXPIRY_SECONDS);
		
	const expiresAt = add(new Date(), {
		seconds: dto.rememberMe
			? COMPUTED.REFRESH_TOKEN_REMEMBER_EXPIRY_SECONDS
			: COMPUTED.REFRESH_TOKEN_EXPIRY_SECONDS,
	});
...
export const refreshAccessToken = async (
	...
	try {
		...
		if (!user)
			throw { status: 401, message: `Unable to find token's owner` };
...
const generateResetPasswordToken = (
	...
	const CHANGE_PASSWORD_EXPIRY = ms(
		`${CONFIG.CHANGE_PASSWORD_EXPIRY_HOURS}h`
	);
...
export const forgotPassword = async (email: string): Promise<void> => {
	try {
		...
		logger.error(
			"Failed to process forgotPassword request due to a service-level error:",
			err
		);
	}
};
