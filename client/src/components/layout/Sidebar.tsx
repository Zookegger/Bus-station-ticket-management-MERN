    ...
    <Box sx={{ ...sx }}>
      ...
      >
        ...
        {!isCollapsed && (
          <>
            <ListItemText
              primary={`${user?.fullName ?? user?.firstName ?? "undefined"}`}
              sx={{
            ...
            <ArrowDownIcon
              sx={{
                color: "white",
                transform: open ? "rotate(180deg)" : "rotate(0deg)", // Rotate 180 degrees when open
          ...
          return (
            <React.Fragment key={item.id}>
              <ListItem disablePadding sx={{ px: 2, mb: 0.5 }}>
                <Tooltip
                  title={<Typography variant="body2">{item.tips}</Typography>}
                  placement="right"
                  id={item.id.toString()}
                  slotProps={{}}
                >
